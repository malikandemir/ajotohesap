...
export default {
  ...
  methods: {
    ...
    update() {
        let self = this;
        axios.post(  '/api/expenses/' + self.$route.params.id + '?token=' + localStorage.getItem("api_token"),
        {
            _method: 'PUT',
  ...
  mounted: function(){
    let self = this;
    axios.get(  '/api/expenses/' + self.$route.params.id + '/edit?token=' + localStorage.getItem("api_token"))
    .then(function (response) {
